...
def _pca(X0, keepdims=None):
    ...
    I = I[:keepdims]
    eigval = eigval[I]
    if not np.all(np.isfinite(eigval)) or np.any(eigval <= 0.0):
        raise ValueError(
...
class WhitenFlow(Flow):
    def __init__(self, X0, keepdims=None, whiten_inverse=True):
        ...
        # Keep jacobian term as buffer so .to(device) in DDP moves it with the module.
        self.register_buffer("jacobian_xz", -torch.sum(torch.log(self.std)))
