...
class CDFTransform(Flow):
    ...
    def _distribution_for_input(self, x):
        """Return a device-aligned distribution for this input tensor."""
        if isinstance(self.distribution, torch.distributions.Normal):
            return torch.distributions.Normal(
                self.distribution.loc.to(device=x.device, dtype=x.dtype),
                self.distribution.scale.to(device=x.device, dtype=x.dtype),
            )
        return self.distribution

    def _forward(self, x, *args, **kwargs):
        distribution = self._distribution_for_input(x)
        y = distribution.cdf(x)
        if self._eps is not None:
            y = y.clamp(self._eps, 1. - self._eps)
        logdet = distribution.log_prob(x)
        if self._eps is not None:
    ...
    def _inverse(self, x, *args, **kwargs):
        ...
        y = distribution.icdf(x)
        logdet = -distribution.log_prob(y)
        if self._eps is not None:
            logdet = logdet.clamp_min(-1/self._eps)
        return y, logdet.sum(dim=-1, keepdim=True)
