...
def make_allegro_config_dict(**kwargs):
    ...
    if GNN_scope == "atomwise":
        dict = base_dict.copy()  # 先复制一个 base_dict，避免修改原字典
        dict.update(atomwise_dict)
...
class NequipWrapper(torch.nn.Module):
    ...
    def forward(self, x):  #this takes as input just the cartesian coordinates of the atoms.
        if not torch.isfinite(x).all():
            ...
            raise RuntimeError(f"NequipWrapper input x contains non-finite values: {bad}")

        absmax = x.detach().abs().max().item()
        if absmax > 1000.0:
            raise RuntimeError(f"NequipWrapper input x has extreme magnitude (absmax={absmax:.4e})")

        batchsize = x.shape[0]
        n_cart_atoms = x.shape[1]
        distances = torch.cdist(x, x)
        if not torch.isfinite(distances).all():
            bad = (~torch.isfinite(distances)).sum().item()
            raise RuntimeError(f"cdist(x, x) produced non-finite values: {bad}")
        ...
        if len(indices) == 0:
            raise RuntimeError("No edges found within cutoff; cannot build graph for NequIP/Allegro.")
        ...
        try:
            ...
            # Torch checkpoint uses this internal exception to short-circuit recomputation.
            # It must propagate untouched; wrapping it breaks backward.
            if exc.__class__.__name__ == "_StopRecomputationError":
                raise
            edge_count = edge_index_batch.shape[1]
            raise RuntimeError(
                f"GNN forward failed; batchsize={batchsize}, n_cart_atoms={n_cart_atoms}, "
                f"edges={edge_count}, cutoff={self.cutoff}, x_absmax={absmax:.4e}"
            ) from exc
