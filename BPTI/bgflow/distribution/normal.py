...
class TruncatedNormalDistribution(Energy, Sampler):
    ...
    def __init__(
        ...
        if sampling_method == "rejection":
            ...
            standard_normal = self._standard_normal_dist()
            self.register_buffer("_cdf_lower_bound", standard_normal.cdf(alpha.detach()))
            self.register_buffer("_cdf_upper_bound", standard_normal.cdf(beta.detach()))
        else:
            raise ValueError(f'Unknown sampling method "{sampling_method}"')

    def _standard_normal_dist(self):
        return torch.distributions.normal.Normal(
            torch.zeros((), device=self._mu.device, dtype=self._mu.dtype),
            torch.ones((), device=self._mu.device, dtype=self._mu.dtype),
        )
    ...
    def _icdf_sampling(self, n_samples, temperature):
        ...
        x = self._standard_normal_dist().icdf(r) * sigma + self._mu
        return x
    ...
    def icdf(self, x):
        ...
        return self._standard_normal_dist().icdf(r) * self._sigma + self._mu

    def cdf(self, x):
        return (self._standard_normal_dist().cdf((x - self._mu)/self._sigma) - self._cdf_lower_bound)/self.Z

    def log_prob(self, x):
        return self._standard_normal_dist().log_prob((x - self._mu)/self._sigma) - torch.log(self.Z * self._sigma)
