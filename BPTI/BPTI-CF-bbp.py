...
BLOCKWISE_COUPLING_BLOCKS = os.environ.get("BLOCKWISE_COUPLING_BLOCKS")
BLOCKWISE_COUPLING_BLOCKS = None if BLOCKWISE_COUPLING_BLOCKS in (None, "", "none", "None") else int(BLOCKWISE_COUPLING_BLOCKS)
BATCHED_JACOBIAN_EVAL = env_flag("BATCHED_JACOBIAN_EVAL", False)
JACOBIAN_EVAL_BATCH_SIZE = os.environ.get("JACOBIAN_EVAL_BATCH_SIZE")
JACOBIAN_EVAL_BATCH_SIZE = (
    None if JACOBIAN_EVAL_BATCH_SIZE in (None, "", "none", "None") else int(JACOBIAN_EVAL_BATCH_SIZE)
)
if JACOBIAN_EVAL_BATCH_SIZE is not None:
    BATCHED_JACOBIAN_EVAL = True
...
def configure_resume_architecture(checkpoint):
    global CONDITIONER, BLOCKWISE_COUPLING_BLOCKS, BATCHED_JACOBIAN_EVAL, JACOBIAN_EVAL_BATCH_SIZE

    if checkpoint is None:
        return

    config = checkpoint.get("config", {})
    checkpoint_conditioner = config.get("conditioner")
    ...
    checkpoint_blocks = config.get("blockwise_coupling_blocks")
    if checkpoint_blocks is not None:
        checkpoint_blocks = int(checkpoint_blocks)
    if "BLOCKWISE_COUPLING_BLOCKS" not in os.environ:
        BLOCKWISE_COUPLING_BLOCKS = checkpoint_blocks
        rank_print(f"resume: using blockwise_coupling_blocks={BLOCKWISE_COUPLING_BLOCKS!r} from checkpoint")
    elif BLOCKWISE_COUPLING_BLOCKS != checkpoint_blocks:
        raise ValueError(
            f"Checkpoint was saved with blockwise_coupling_blocks={checkpoint_blocks!r}, "
            f"but current BLOCKWISE_COUPLING_BLOCKS={BLOCKWISE_COUPLING_BLOCKS!r}."
        )

    checkpoint_batched_jacobian = bool(config.get("batched_jacobian_eval", False))
    if "BATCHED_JACOBIAN_EVAL" not in os.environ:
        BATCHED_JACOBIAN_EVAL = checkpoint_batched_jacobian
        rank_print(f"resume: using batched_jacobian_eval={BATCHED_JACOBIAN_EVAL!r} from checkpoint")

    checkpoint_jacobian_batch_size = config.get("jacobian_eval_batch_size")
    if checkpoint_jacobian_batch_size is not None:
        checkpoint_jacobian_batch_size = int(checkpoint_jacobian_batch_size)
    if "JACOBIAN_EVAL_BATCH_SIZE" not in os.environ:
        JACOBIAN_EVAL_BATCH_SIZE = checkpoint_jacobian_batch_size
...
def KLloss(
    ...
    mapped_samples, dlogp = evaluate_flow(myflow, prior_samples)
...
def validate_gpu_optimization_config():
    if BLOCKWISE_COUPLING_BLOCKS is not None and BLOCKWISE_COUPLING_BLOCKS < 2:
        raise ValueError("BLOCKWISE_COUPLING_BLOCKS must be None or an integer >= 2")
    if BATCHED_JACOBIAN_EVAL and JACOBIAN_EVAL_BATCH_SIZE is None:
        raise ValueError("Set JACOBIAN_EVAL_BATCH_SIZE to enable BATCHED_JACOBIAN_EVAL")
    if JACOBIAN_EVAL_BATCH_SIZE is not None and JACOBIAN_EVAL_BATCH_SIZE < 1:
        raise ValueError("JACOBIAN_EVAL_BATCH_SIZE must be None or an integer >= 1")


def add_torsion_couplings(builder, ca_rep, n_rounds=4):
    n_torsions = builder.current_dims[TORSIONS][-1]
    if BLOCKWISE_COUPLING_BLOCKS is None:
    ...
    block_indices = [np.arange(i, n_torsions, BLOCKWISE_COUPLING_BLOCKS) for i in range(BLOCKWISE_COUPLING_BLOCKS)]
    block_indices = [indices for indices in block_indices if len(indices) > 0]
    block_names = [f"T_BLOCK_{i}" for i in range(len(block_indices))]
    torsion_blocks = builder.add_split(TORSIONS, into=block_names, sizes_or_indices=block_indices)

    for _ in range(n_rounds):
        for block in torsion_blocks:
            conditioning_blocks = tuple(other for other in torsion_blocks if other != block)
            builder.add_condition(
                block,
                on=(*ca_rep, *conditioning_blocks),
                hidden=(128, 128),
                param_groups=("aa", "torsions", "blockwise_coupling"),
            )
    builder.add_merge(torsion_blocks, to=TORSIONS, sizes_or_indices=block_indices)
    return len(torsion_blocks)


def evaluate_flow(flow, samples):
    if not BATCHED_JACOBIAN_EVAL or JACOBIAN_EVAL_BATCH_SIZE is None:
        return flow(samples)

    mapped, dlogp = [], []
    for start in range(0, samples.shape[0], JACOBIAN_EVAL_BATCH_SIZE):
        batch = samples[start : start + JACOBIAN_EVAL_BATCH_SIZE]
        batch_mapped, batch_dlogp = flow(batch)
        mapped.append(batch_mapped)
        dlogp.append(batch_dlogp)
    return torch.cat(mapped, dim=0), torch.cat(dlogp, dim=0)
...
def build_bgmap(dataset_1, system_1, target, training_data):
    ...
    CA_REP = [FIXED]
    n_torsion_blocks = add_torsion_couplings(builder, CA_REP)
    ...
    rank_print(
        ...
        f"blockwise_coupling_blocks={BLOCKWISE_COUPLING_BLOCKS!r} "
        f"(active_blocks={n_torsion_blocks}), "
        f"batched_jacobian_eval={BATCHED_JACOBIAN_EVAL}, "
        f"jacobian_eval_batch_size={JACOBIAN_EVAL_BATCH_SIZE!r}"
    )
...
def run():
    ...
    set_seed(20260529)
    validate_gpu_optimization_config()
        ...
        torch.save(
            {
                ...
                "config": {
                    ...
                    "clipping": CLIPPING,
                    "grad_clip": GRAD_CLIP,
                    "blockwise_coupling_blocks": BLOCKWISE_COUPLING_BLOCKS,
                    "batched_jacobian_eval": BATCHED_JACOBIAN_EVAL,
                    "jacobian_eval_batch_size": JACOBIAN_EVAL_BATCH_SIZE,
                },
            },
            CHECKPOINT_PATH,
        )
